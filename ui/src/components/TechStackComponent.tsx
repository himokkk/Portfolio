...
const TechStackComponent = (() => {
    ...
    function randomSpawnPosition(centerPoint: Vector, jar_size: number, bubble_size: number): Vector  {
        let ballPosition: Vector = {
            x: Math.floor(Math.random() * (jar_size*0.8 - bubble_size)),
            y: Math.floor(Math.random() * (jar_size*0.8 - bubble_size)),
        }
    ...
    function calculateNewVelocity(ballPosition: Vector, centerPoint: Vector, velocity: Vector): Vector {
    ...
    function getClosestBallPositionOnCircle(
        ...
        const distanceToIntersection = distance - Radius + radius;
    ...
    useEffect (() => {
        ...
        Array.from(bubbles).forEach((bubble: HTMLDivElement) => {
            ...
            velocity.y = Math.sqrt(Math.pow(speed, 2) - Math.pow(velocity.x, 2)) * -randomSign;
            ...
            function moveBubble() {
                ...
                if (Math.sqrt(Math.pow(distX, 2) + Math.pow(distY, 2)) >= jar_size / 2 - bubble_size / 2) {
                    let fixedPosition = getClosestBallPositionOnCircle(ballPosition, velocity, centerPoint, jar_size / 2, bubble_size/2);
                    
                    leftPosition -= fixedPosition.x;
                    topPosition -= fixedPosition.y;
